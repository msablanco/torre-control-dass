...
st.set_page_config(page_title="Dass Performance v10.7", layout="wide")
...
if data:
    ...
    def apply_filters(df_target, is_maestro=False):
        ...
        for c in ['Disciplina', 'FRANJA_PRECIO']:
            if c not in t.columns: t[c] = 'SIN DATO'
    ...
    # --- 5. DASHBOARD ---
    st.title(f"📊 Torre de Control Dass v10.7")
    ...
    # Inyectar categorías al Stock para evitar KeyErrors
    stk_snap_ma = stk_snap.merge(df_ma[['SKU', 'Disciplina', 'FRANJA_PRECIO']], on='SKU', how='left') if not stk_snap.empty else pd.DataFrame()
    for col in ['Disciplina', 'FRANJA_PRECIO', 'Cliente_up']:
        if col not in stk_snap_ma.columns: stk_snap_ma[col] = 'SIN DATO'

    k_so, k_si = so_f['Cant'].sum(), si_f['Cant'].sum()
    k_sc = stk_snap_ma[~stk_snap_ma['Cliente_up'].str.contains('DASS')]['Cant'].sum() if not stk_snap_ma.empty else 0
    k_sd = stk_snap_ma[stk_snap_ma['Cliente_up'].str.contains('DASS')]['Cant'].sum() if not stk_snap_ma.empty else 0
    ...
    def render_row(title, so_df, si_df, stk_df, group_col, color_map=None):
        ...
        if not stk_df.empty and group_col in stk_df.columns:
        ...
        if not so_df.empty and group_col in so_df.columns:
        ...
        if not si_df.empty and group_col in si_df.columns:
    ...
    # Fila 3: Arribos (Con blindaje de error)
    st.subheader("🚚 Análisis de Arribos")
    i1, i2, i3, i4 = st.columns([1, 1, 1, 2])
    if not si_f.empty:
        if 'Disciplina' in si_f.columns: i1.plotly_chart(px.pie(si_f.groupby('Disciplina')['Cant'].sum().reset_index(), values='Cant', names='Disciplina', title="Ingresos x Dis", color='Disciplina', color_discrete_map=COLOR_MAP_DIS), use_container_width=True)
        if 'FRANJA_PRECIO' in si_f.columns: i2.plotly_chart(px.pie(si_f.groupby('FRANJA_PRECIO')['Cant'].sum().reset_index(), values='Cant', names='FRANJA_PRECIO', title="Ingresos x Franja"), use_container_width=True)
    
    if not stk_snap_ma.empty and 'Disciplina' in stk_snap_ma.columns:
        sd_dis = stk_snap_ma[stk_snap_ma['Cliente_up'].str.contains('DASS')].groupby('Disciplina')['Cant'].sum().reset_index()
        if not sd_dis.empty: i3.plotly_chart(px.pie(sd_dis, values='Cant', names='Disciplina', title="Stk Dass Part.", color='Disciplina', color_discrete_map=COLOR_MAP_DIS), use_container_width=True)

    if not si_f.empty and 'FRANJA_PRECIO' in si_f.columns:
        i4.plotly_chart(px.bar(si_f.groupby(['Mes', 'FRANJA_PRECIO'])['Cant'].sum().reset_index(), x='Mes', y='Cant', color='FRANJA_PRECIO', title="Arribos Mensuales"), use_container_width=True)
