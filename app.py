...
data = load_data()if data:
    ...
    so_f, si_f, stk_f = apply_filters(so_raw), apply_filters(si_raw), apply_filters(stk_raw)st.title("📊 Torre de Control Dass v11.38")
    ...
    with c1:
        ...
        fig_dis = px.bar(df_dis_bar, x='MES', y='CANT', color='DISCIPLINA', title="Mix Sell In % (Disciplina)", color_discrete_map=COLOR_MAP_DIS, text_auto='.1s')
    ...
    with c2:
        ...
        fig_fra = px.bar(df_fra_bar, x='MES', y='CANT', color='FRANJA_PRECIO', title="Mix Sell In % (Franja)", color_discrete_map=COLOR_MAP_FRA, text_auto='.1s')
