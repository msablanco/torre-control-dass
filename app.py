...
if data:
    ...
    # --- LÓGICA DE STOCK SEGÚN SUMAR.SI(F:F; "wholesale"; B:B) ---
    ...
    # Stock Cliente: Registros que coinciden exactamente con 'WHOLESALE' en Col F
    val_wholesale = stk_snap[stk_snap['CLIENTE_UP'] == 'WHOLESALE']['CANT'].sum()

    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Sell Out", f"{so_f['CANT'].sum():,.0f}")
    k2.metric("Sell In", f"{si_f['CANT'].sum():,.0f}")
    k3.metric("Stock Dass", f"{val_dass:,.0f}")
    k4.metric("Stock Cliente", f"{val_wholesale:,.0f}")
    ...
    if val_wholesale > 0:
        c3.plotly_chart(px.pie(stk_snap[stk_snap['CLIENTE_UP'] == 'WHOLESALE'].groupby('DISCIPLINA')['CANT'].sum().reset_index(), values='CANT', names='DISCIPLINA', title="Stock Cliente", color='DISCIPLINA', color_discrete_map=COLOR_MAP_DIS), use_container_width=True)
    if not si_f.empty:
    ...
    if val_wholesale > 0:
        f3.plotly_chart(px.pie(stk_snap[stk_snap['CLIENTE_UP'] == 'WHOLESALE'].groupby('FRANJA_PRECIO')['CANT'].sum().reset_index(), values='CANT', names='FRANJA_PRECIO', title="Stock Cliente (Franja)", color='FRANJA_PRECIO', color_discrete_map=COLOR_MAP_FRA), use_container_width=True)
    if not si_f.empty:
        f4.plotly_chart(px.bar(si_f.groupby(['MES', 'FRANJA_PRECIO'])['CANT'].sum().reset_index(), x='MES', y='CANT', color='FRANJA_PRECIO', title="Sell In por Franja", color_discrete_map=COLOR_MAP_FRA), use_container_width=True)
    ...
    h_sc = stk_h_all[stk_h_all['CLIENTE_UP'] == 'WHOLESALE'].groupby('MES')['CANT'].sum().reset_index().rename(columns={'CANT': 'Stock Cliente'})
    ...
    t_stk_c = stk_snap[stk_snap['CLIENTE_UP'] == 'WHOLESALE'].groupby('SKU')['CANT'].sum().reset_index(name='Stock Cliente')
