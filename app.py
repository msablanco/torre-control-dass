...
st.set_page_config(page_title="Dass Performance v11.22 - RESTORE", layout="wide")
...
if data:
    ...
    def clean_df(name):
        ...
        col_cant = next((c for c in df.columns if any(x in c.upper() for x in ['UNID', 'CANT'])), 'Cant')
        df['Cant'] = pd.to_numeric(df[col_cant], errors='coerce').fillna(0)
        col_fecha = next((c for c in df.columns if any(x in c.upper() for x in ['FECHA', 'VENTA', 'MES'])), 'Fecha')
    ...
    with tab_control:
        ...
        # LOGICA VOLVER A ATRAS: Dass vs Clientes
        ...
        # --- FILA 1: DISCIPLINAS (TORTAS + BARRAS) ---
        st.subheader("📌 Análisis por Disciplina")
        c1, c2, c3, c4 = st.columns([1, 1, 1, 2])
        c1.plotly_chart(px.pie(df_stk_dass.groupby('Disciplina')['Cant'].sum().reset_index(), values='Cant', names='Disciplina', title="Stk Dass", color_discrete_map=COLOR_MAP_DIS), use_container_width=True)
        ...
        c3.plotly_chart(px.pie(df_stk_clie.groupby('Disciplina')['Cant'].sum().reset_index(), values='Cant', names='Disciplina', title="Stk Cliente", color_discrete_map=COLOR_MAP_DIS), use_container_width=True)
        c4.plotly_chart(px.bar(si_f.groupby(['Mes', 'Disciplina'])['Cant'].sum().reset_index(), x='Mes', y='Cant', color='Disciplina', title="Sell In por Mes", color_discrete_map=COLOR_MAP_DIS), use_container_width=True)

        # --- FILA 2: FRANJAS (TORTAS + BARRAS) ---
        st.subheader("💰 Análisis por Franja")
        f1, f2, f3, f4 = st.columns([1, 1, 1, 2])
        f1.plotly_chart(px.pie(df_stk_dass.groupby('FRANJA_PRECIO')['Cant'].sum().reset_index(), values='Cant', names='FRANJA_PRECIO', title="Stk Dass"), use_container_width=True)
        ...
        f3.plotly_chart(px.pie(df_stk_clie.groupby('FRANJA_PRECIO')['Cant'].sum().reset_index(), values='Cant', names='FRANJA_PRECIO', title="Stk Cliente"), use_container_width=True)
    ...
    with tab_intel:
        ...
        st.info("Utilice los filtros laterales para analizar SKUs específicos.")
