...
st.set_page_config(
    ...
    page_icon="logo_fila.png",  # Icono en la pestaña del navegador
    layout="wide"
)
...
if data:
    ...
    # --- 3. BARRA LATERAL (LOGO + FILTROS) ---
    try:
        st.sidebar.image("logo_fila.png", use_container_width=True)
    except:
        st.sidebar.info("Añade 'logo_fila.png' al repositorio para ver el logo.")

    st.sidebar.header("🔍 Inteligencia de Datos")
    search_sku = st.sidebar.text_input("🎯 Buscar SKU / Modelo").upper()
    
    # Sell Out e In Raw para filtros de mes
    so_raw = pd.DataFrame() # placeholder
    ...
    # Lógica de fecha máxima independiente por cliente
    ...
    # --- 7. GRÁFICOS DE DISCIPLINA ---
    st.divider()
    st.subheader("📊 Análisis por Disciplina")
    c1, c2, c3, c4 = st.columns([1, 1, 1, 2])
    with c1: st.plotly_chart(px.pie(snap_dass.groupby('DISCIPLINA')['CANT'].sum().reset_index(), values='CANT', names='DISCIPLINA', title="Stock Dass", color_discrete_map=COLOR_MAP_DIS), use_container_width=True)
    ...
    with c4: st.plotly_chart(px.bar(si_f.groupby(['MES', 'DISCIPLINA'])['CANT'].sum().reset_index(), x='MES', y='CANT', color='DISCIPLINA', title="Evolución Sell In", color_discrete_map=COLOR_MAP_DIS, text_auto='.2s'), use_container_width=True)

    # --- 8. HISTÓRICO ---
    st.divider()
    st.subheader("📈 Tendencias y Evolución Temporal")
    h_so = apply_filters(so_raw, False).groupby('MES')['CANT'].sum().reset_index(name='SO')
    h_si = apply_filters(si_raw, False).groupby('MES')['CANT'].sum().reset_index(name='SI')
    ...
    # Filtrar solo SKUs con algún movimiento
    ...
    st.error("Por favor, verifica la conexión con los archivos de Google Drive.")
