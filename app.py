...
st.set_page_config(page_title="Dass Performance v8.6", layout="wide")
...
if data:
    # --- 2. PROCESAMIENTO MAESTRO (Base de Columnas) ---
    df_ma = data.get('Maestro_Productos', pd.DataFrame()).copy()
    if not df_ma.empty:
        df_ma['SKU'] = df_ma['SKU'].astype(str).str.strip().str.upper()
        # Buscamos Franja con flexibilidad
        ...
        st.error("No se encontró el archivo Maestro_Productos.")
        st.stop()

    # --- 3. PROCESAMIENTO SELL OUT (Aislado para Total 457k) ---
    so_raw = data.get('Sell_out', pd.DataFrame()).copy()
    so_stats = pd.DataFrame(columns=['SKU', 'Cliente', 'Sell out Total', 'Max_Mensual_3M'])
    if not so_raw.empty:
        ...
        # Máximo Mensual (Suma primero, Máximo después)
        ...
        # Totales por SKU y Cliente para filtros
    ...
    st_cli_grp = pd.DataFrame(columns=['SKU', 'Cliente', 'Stock Clientes'])
    ...
    clis_all = sorted(list(set(so_stats['Cliente'].unique().tolist() + st_cli_grp['Cliente'].unique().tolist())))
    ...
    # Agrupar para Merge final
    so_final = so_stats.groupby('SKU')[['Sell out Total', 'Max_Mensual_3M']].sum().reset_index()
    stk_cli_final = st_cli_grp.groupby('SKU')['Stock Clientes'].sum().reset_index()

    # Merge Final
    df = df_ma.merge(so_final, on='SKU', how='left').merge(stk_cli_final, on='SKU', how='left').merge(st_dass_grp, on='SKU', how='left').fillna(0)
    ...
    # --- 6. VISUALIZACIÓN ---
    st.title("📊 Torre de Control Dass v8.6")
    ...
    # Columnas finales garantizadas para evitar KeyError
    cols_finales = ['SKU', 'Descripcion', 'Disciplina', 'FRANJA_PRECIO', 'Sell out Total', 'Max_Mensual_3M', 'Stock Clientes', 'MOS', 'Stock Dass']
    
    st.dataframe(
        df_ver[cols_finales].sort_values('Sell out Total', ascending=False).style.format({
