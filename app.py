...
if data:
    # A. Maestro de Productos (con Emprendimiento)
    ...
    if not df_ma.empty:
        ...
        # Buscamos la columna Emprendimiento, si no existe la creamos vacía
        if 'EMPRENDIMIENTO' not in df_ma.columns:
            df_ma['EMPRENDIMIENTO'] = 'GENERAL'
        else:
            df_ma['EMPRENDIMIENTO'] = df_ma['EMPRENDIMIENTO'].fillna('GENERAL').astype(str).str.upper()
            
        df_ma = df_ma.drop_duplicates(subset=['SKU'])
        df_ma['BUSQUEDA'] = df_ma['SKU'] + " " + df_ma['DESCRIPCION'].fillna('').astype(str).str.upper()

    # B. Procesamiento de Ventas
    ...
    # D. Filtros Laterales (Agregado EMPRENDIMIENTO)
    ...
    emp_op = sorted([str(x) for x in df_ma['EMPRENDIMIENTO'].unique()]) if not df_ma.empty else []
    f_emp = st.sidebar.multiselect("🚀 Emprendimiento", emp_op)
    ...
    def apply_filters(df, is_stock=False):
        ...
        temp = df.merge(df_ma[['SKU', 'DISCIPLINA', 'FRANJA_PRECIO', 'EMPRENDIMIENTO', 'DESCRIPCION', 'BUSQUEDA']], on='SKU', how='left')
        temp[['DISCIPLINA', 'FRANJA_PRECIO', 'EMPRENDIMIENTO']] = temp[['DISCIPLINA', 'FRANJA_PRECIO', 'EMPRENDIMIENTO']].fillna('SIN CATEGORIA')
    ...
    # H. Matriz de Datos (Incluyendo Emprendimiento)
    st.divider()
    ...
    df_final = df_ma[['SKU', 'DESCRIPCION', 'DISCIPLINA', 'FRANJA_PRECIO', 'EMPRENDIMIENTO']].merge(t_so, on='SKU', how='left').merge(t_si, on='SKU', how='left').merge(t_sd, on='SKU', how='left').merge(t_sc, on='SKU', how='left').fillna(0)
    ...
    st.warning("⚠️ Esperando datos... Asegúrate de que el Maestro de Productos tenga la columna 'EMPRENDIMIENTO'.")
